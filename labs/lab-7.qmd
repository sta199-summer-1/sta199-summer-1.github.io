...
::: callout-important
This lab is due Tuesday, March 28 at 11:59pm.
:::
...
along with either `if_else()` or `case_when()` to re-level the variable and then `fct_relevel()` to
re-order the levels. (See Lab 2 for an example of using `if_else` and HW 1 Exercise 4 for an example of using `fct_relevel`.)
